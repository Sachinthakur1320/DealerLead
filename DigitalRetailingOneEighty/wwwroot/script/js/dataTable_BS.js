	...
	var DataTable = window.DataTable = function (container, data, schema) {
			...
			for (var i = 0; i < data.length; i++) {
				...
				for (field = 0; field < fields.length; field++) {
					...
					if (typeof fields[field].className === 'function') {
						r.push(fields[field].className(data[i][fields[field].index], data[i]));
					}
					if (fields[field].responsiveClass) {
						r.push(' ' + fields[field].responsiveClass);
					}
					if (typeof fields[field].hideMousePointer === 'function') {
					    if (fields[field].hideMousePointer(null, data[i])) {
					        r.push(' no-pointer')
					    }
					};
					r.push('"');
					...
					if (fields[field] && fields[field].view) {
						var html = fields[field].view(data[i][fields[field].index], data[i]);
						if (html) {
							r.push(html);
						}
					} else {
		...
		var renderHeader = function (table, fields) {
			...
			for (field = 0; field < fields.length; field++) {
				...
				r.push('<th');
				if (typeof fields[field].responsiveClass) {
					r.push(' class="' + fields[field].responsiveClass + '"');
				}
				if (fields[field] && typeof fields[field].header === 'string') {
					r.push('>' + fields[field].header + '</th>')
		...
		var self = {
					...
					if (self.schema.group && self.data[0].key !== undefined) {
						...
						for (var i = 0; i < self.data.length; i++) {
							...
							if (typeof self.schema.group.header == "function") {
								...
								tr = $('<tr><td colspan="' + fields.length + '"><span>' + self.data[i].key + '</span></td></tr>');
							}
							tr.addClass("table-group").addClass("collapsable");
			...
			collapseAll: function () {
				if (_table) {
					_table.find('tr.table-group:not(.collapsed)').click();
				}
			},
			expandAll: function () {
				if (_table) {
					_table.find('tr.table-group.collapsed').click();
				}
			},
			showLoading: function (state, message) {
				if (state && (!_maskVisible | (message && message !== _lastMessage))) {
					var messageInputed = !!message;
					_lastMessage = message;
					_maskVisible = true;
					message = message || '<img style="float:left" src="/webasp/images/oespinner-trans.gif"/>';
					if (!_mask) {
						_mask = $('<div />')
							.css({
								'z-index': 500,
					...
					if (messageInputed) {
						_message.css({
							...
							'border-radius': '6px',
							'box-shadow': 'rgba(0, 0, 0, 0.298039) 0px',
							'background': '#fff'
						})
					} else {
						_message.css({
					...
					_mask.height(_parent.height()).width(_parent.width())
						.fadeIn(250);
					var maskOffset = _mask.position();
					_message.fadeIn(250)
						.html(message)
						.css('top', maskOffset.top + 100).css('left', maskOffset.left + ((_mask.width() - _message.width()) / 2));
				} else if (state && _maskVisible) {
					_mask.height(_parent.height()).width(_parent.width())
					if (message && _message) {
						var maskOffset = _mask.position();
						_message.css('top', maskOffset.top + 100).css('left', maskOffset.left + ((_mask.width() - _message.width()) / 2));
					}
				} else if (!state && _maskVisible) {
					_maskVisible = false;
					if (_mask) { _message.fadeOut(250); _mask.fadeOut(250); }
				}
			}
			/*
